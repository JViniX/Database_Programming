...
PRINT ('------------- Iniciado às '+ cast(getdate() as varchar(20))+' -----------------')
--SET @DATAATUAL =CAST(@ANO_ATUAL AS CHAR(4))+'-'+CAST(@MES_ATUAL AS CHAR(2))+'-01'
--SET @DATAATUAL = DATEADD(D,1-DAY(@DATAATUAL),DATEADD(M,0,CONVERT(CHAR(8),@DATAATUAL,112))) 
--select @DATAATUAL
...
END
PRINT ('------------- Terminado às '+ cast(getdate() as varchar(20))+' -----------------')
