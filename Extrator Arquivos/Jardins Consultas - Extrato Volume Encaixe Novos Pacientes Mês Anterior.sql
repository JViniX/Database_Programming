...
Set @Titulo = 'Jardins Consultas Vol Encaixe novos Pacientes: '+ convert(varchar(2),month(@MesAno))+'/'+convert(varchar(4),year(@MesAno))

EXEC sp_makewebtask 
	@outputfile = 'D:\Extratos\Jardins Consultas Vol Encaixe novos Pacientes.xls',  
	@colheaders =1, 
...
select distinct(pac.pac_reg),i.smm_osm_serie "OS Serie", i.smm_osm "OS Num", i.smm_sfat "Status Fat", i.smm_str "Cod. Setor", s.str_nome "Setor", 
 ...
 ''R$ ''+replace(cast(i.smm_vlr as numeric(10,2)),''.'','','') Valor, i.smm_usr_login_lanc, fle.fle_marc_extra "Encaixe"
