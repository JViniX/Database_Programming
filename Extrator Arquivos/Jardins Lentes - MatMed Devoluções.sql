...
Set @Titulo = 'Jardins Lentes Mat/Med Devoluções: '+ convert(varchar(2),month(@MesAno))+'/'+convert(varchar(4),year(@MesAno))

EXEC sp_makewebtask 
	@outputfile = 'D:\Extratos\Jardins Lentes MatMed Devoluções.xls',  
	@colheaders =1, 
...
'SELECT v.mma_sma_serie "Solic Serie", v.mma_sma_num as "No.Solic", sma_data as "Data Solic", s.sma_usr_login_sol as "Usr. Solic.", al.sba_nome as "Almoxarifado",
			...
			WHERE V.MMA_TIPO_ES = ''E''
			AND MMA_TIPO_OPERACAO = ''E4''	
