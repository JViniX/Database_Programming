...
Set @Titulo = 'Jardins Consulta Mat/Med: '+ convert(varchar(2),month(@MesAno))+'/'+convert(varchar(4),year(@MesAno))

EXEC sp_makewebtask 
	@outputfile = 'D:\Extratos\Jardins Consulta MatMed.xls',  
	@colheaders =1, 
...
'SELECT v.mma_sma_serie "Solic Serie", v.mma_sma_num as "No.Solic", sma_data as "Data Solic", s.sma_usr_login_sol as "Usr. Solic.", al.sba_nome as "Almoxarifado",
			...
			AND (MMA_STR_COD = ''JCO'' OR MMA_STR_COD = ''JCH'' OR MMA_STR_COD = ''JCM'')
