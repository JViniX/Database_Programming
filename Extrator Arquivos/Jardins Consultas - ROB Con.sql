...
Set @Titulo = 'Jardins Consulta ROB Conv: '+ convert(varchar(2),month(@MesAno))+'/'+convert(varchar(4),year(@MesAno))

EXEC sp_makewebtask 
	@outputfile = 'D:\Extratos\Jardins Consulta ROB Conv.xls',  
	@colheaders =1, 
...
select i.smm_osm_serie "OS Serie", i.smm_osm "OS Num", i.smm_sfat "Status Fat", i.smm_str "Cod. Setor", s.str_nome "Setor", 
 ...
 ''R$ ''+replace(cast(i.smm_vlr as numeric(10,2)),''.'','','') Valor, i.smm_usr_login_lanc
...
inner join pac on i.smm_pac_reg = pac.pac_reg
			...
			AND (I.SMM_STR = ''JCO'' OR I.SMM_STR = ''JCH'' OR I.SMM_STR = ''JCM'' OR I.SMM_STR = ''JSC'')
