-- ROB Conv
/*DECLARACAO DE VARIAVEIS*/
Declare @Titulo Varchar(50)
Declare @MesAno DateTime

/*PEGA O ANO E MES ATUAL*/
Set @MesAno = DATEADD(D,1-DAY(GETDATE()),DATEADD(M,-1,CONVERT(CHAR(8),GETDATE(),112)))
Set @Titulo = 'Consultas ROB-HON Conv: '+ convert(varchar(2),month(@MesAno))+'/'+convert(varchar(4),year(@MesAno))

EXEC sp_makewebtask 
	@outputfile = 'D:\Extratos\Consultas ROB-HON Conv.xls',  
	@colheaders =1, 
	@FixedFont=0,@lastupdated=0,@resultstitle=@Titulo,
	@query = '
SELECT O.OSM_SERIE "Série", O.OSM_NUM "Número", I.SMM_SFAT "Status Fat", convert(varchar, o.OSM_DTHR, 103) Data, convert(varchar, o.OSM_DTHR, 108) Hora, I.SMM_COD "Código", 
		P.SMK_NOME "Descrição", i.smm_med "CRM", crm.psv_nome "Prestador", C.CNV_NOME "Convênio", I.SMM_QT "Qtd.", replace(cast(i.smm_vlr as numeric(10,3)),''.'','','') "Valor", c.CNV_CAIXA_FATURA "Cobraca", I.SMM_TIPO_FATURA "TipoFatura", H.HON_SEQ "Rateio No.", 

cc1.ccr_tit "Med Exec", isnull(h.hon_cc1, ''99999'') "Conta Med Exec", replace(cast(ISNULL(H.HON_PC1,0) as numeric(10,2)),''.'','','')+''%'' "% Rat Med Exec", ''R$''+replace(cast(ISNULL(H.HON_VL1,0) as numeric(10,2)),''.'','','') "R$ Rat Med Exec", replace(cast(ISNULL(((ISNULL(H.HON_PC1,0)*(I.SMM_VLR - (ISNULL(H.HON_VL1,0) + ISNULL(H.HON_VL2,0) + ISNULL(H.HON_VL3,0) + ISNULL(H.HON_VL4,0) + ISNULL(H.HON_VL5,0) + ISNULL(H.HON_VL6,0)))/100)+(ISNULL(H.HON_VL1,0))),0) as numeric(10,3)),''.'','','')   "R$ Hono Med Exec",
cc2.ccr_tit "Med Aux", isnull(h.hon_cc2, ''99999'') "Conta Med Aux", replace(cast(ISNULL(H.HON_PC2,0) as numeric(10,2)),''.'','','')+''%'' "% Rat Med Aux", ''R$''+replace(cast(ISNULL(H.HON_VL2,0) as numeric(10,2)),''.'','','') "R$ Rat Med Aux", replace(cast(ISNULL(((ISNULL(H.HON_PC2,0)*(I.SMM_VLR - (ISNULL(H.HON_VL1,0) + ISNULL(H.HON_VL2,0) + ISNULL(H.HON_VL3,0) + ISNULL(H.HON_VL4,0) + ISNULL(H.HON_VL5,0) + ISNULL(H.HON_VL6,0)))/100)+(ISNULL(H.HON_VL2,0))),0) as numeric(10,3)),''.'','','')  "R$ Hono Med Aux",
cc6.ccr_tit "Med solic", isnull(h.hon_cc6, ''99999'') "Conta Med solic", replace(cast(ISNULL(H.HON_PC6,0) as numeric(10,2)),''.'','','')+''%'' "% Rat Med solic", ''R$''+replace(cast(ISNULL(H.HON_VL6,0) as numeric(10,2)),''.'','','') "R$ Rat Med solic", replace(cast(ISNULL(((ISNULL(H.HON_PC6,0)*(I.SMM_VLR - (ISNULL(H.HON_VL1,0) + ISNULL(H.HON_VL2,0) + ISNULL(H.HON_VL3,0) + ISNULL(H.HON_VL4,0) + ISNULL(H.HON_VL5,0) + ISNULL(H.HON_VL6,0)))/100)+(ISNULL(H.HON_VL6,0))),0) as numeric(10,3)),''.'','','')  "R$ Hono Med Solic",

--''R$''+
replace(cast(ISNULL(
			(((ISNULL(H.HON_PC1,0)*(I.SMM_VLR - (ISNULL(H.HON_VL1,0) + ISNULL(H.HON_VL2,0) + ISNULL(H.HON_VL3,0) + ISNULL(H.HON_VL4,0) + ISNULL(H.HON_VL5,0) + ISNULL(H.HON_VL6,0)))/100)+(ISNULL(H.HON_VL1,0))))+
			(((ISNULL(H.HON_PC2,0)*(I.SMM_VLR - (ISNULL(H.HON_VL1,0) + ISNULL(H.HON_VL2,0) + ISNULL(H.HON_VL3,0) + ISNULL(H.HON_VL4,0) + ISNULL(H.HON_VL5,0) + ISNULL(H.HON_VL6,0)))/100)+(ISNULL(H.HON_VL2,0))))+
			(((ISNULL(H.HON_PC6,0)*(I.SMM_VLR - (ISNULL(H.HON_VL1,0) + ISNULL(H.HON_VL2,0) + ISNULL(H.HON_VL3,0) + ISNULL(H.HON_VL4,0) + ISNULL(H.HON_VL5,0) + ISNULL(H.HON_VL6,0)))/100)+(ISNULL(H.HON_VL6,0)))) 
,0) as numeric(10,3)),''.'','','') 
"Total Pag Hon R$"

from osm as o
inner join smm as i on I.SMM_OSM = O.OSM_NUM AND I.SMM_OSM_SERIE = O.OSM_SERIE	
inner join hon as h on I.SMM_HON_SEQ = H.HON_SEQ
inner join ccr as cc1 on cc1.ccr_cod = isnull(h.hon_cc1, ''99999'')
inner join ccr as cc2 on cc2.ccr_cod = isnull(h.hon_cc2, ''99999'')
inner join ccr as cc6 on cc6.ccr_cod = isnull(h.hon_cc6, ''99999'')
inner join SMK as P on I.SMM_COD = P.SMK_COD AND I.SMM_TPCOD = P.SMK_TIPO
inner join CNV as C on O.OSM_CNV = C.CNV_COD 	
inner join psv crm on crm.psv_cod = i.smm_med		

			WHERE  O.OSM_DTHR >= DATEADD(D,1-DAY(GETDATE()),DATEADD(M,-1,CONVERT(CHAR(8),GETDATE(),112))) 
			AND O.OSM_DTHR < DATEADD(D,1-DAY(getdate()),CONVERT(CHAR(8),getdate(),112))
 
			and (i.smm_pacote = ''C'' or i.smm_pacote is null)
			AND (I.SMM_SFAT = ''F'' or I.SMM_SFAT = ''P'')
			AND I.SMM_TIPO_FATURA = ''M''
			AND C.CNV_CAIXA_FATURA = ''F''
			AND C.CNV_COD not in (''SUS'', ''SAD'')
			AND (I.SMM_STR = ''C1''
				OR I.SMM_STR = ''C2''
				OR I.SMM_STR= ''C3''
				OR I.SMM_STR = ''C4''
				OR I.SMM_STR = ''C5''
				OR I.SMM_STR = ''C6''
				OR I.SMM_STR = ''CAS''
				OR I.SMM_STR = ''CTD''
				OR I.SMM_STR = ''CFR''
				OR I.SMM_STR = ''CMU''
				OR I.SMM_STR = ''CN1''
				OR I.SMM_STR = ''CU1''
				OR I.SMM_STR = ''CU2''
				OR I.SMM_STR = ''GON''
				OR I.SMM_STR = ''PC2''
				OR I.SMM_STR = ''PR1''
				OR I.SMM_STR = ''TS'')
			
			AND SMM_PAC_REG NOT IN (SELECT PAC_REG 
						FROM PAC 
						WHERE PAC_NOME LIKE ''%TESTE %'')

order by smm_osm_serie, smm_osm'
