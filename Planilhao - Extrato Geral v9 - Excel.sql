...
Set @Titulo = 'Extrato Geral v9 - CC e EXL 2014-1s2017.xls' 
...
O.OSM_SERIE "Série", O.OSM_NUM "Número", i.smm_num Seq_OS, I.SMM_SFAT "Status Fat", convert(varchar, o.OSM_DTHR, 103) Data, convert(varchar, o.OSM_DTHR, 108) Hora, 
		...
		Case
			when 
				i.smm_pacote = ''P'' or I.SMM_SFAT in (''A'', ''C'')
				then ''0''
			else I.SMM_QT 
		End Qtd,
		Case
		...
		End Recebido, 
		i.smm_tipo_fatura "Faturado Para", i.smm_usr_login_lanc Usuário, I.SMM_HON_SEQ "Rateio No."
...
left join  V_EXT_TOT v on v.V_OSM_SERIE = i.SMM_OSM_SERIE AND v.V_OSM_NUM = i.SMM_OSM and v.V_SMM_NUM = i.SMM_NUM 
			...
			WHERE  
                ...
                and o.osm_dthr >= ''2014-01-01 00:00'' and O.OSM_DTHR < ''2017-07-01 00:00''
...
from #Extrato I
left join hon as h on "Rateio No." = H.HON_SEQ
