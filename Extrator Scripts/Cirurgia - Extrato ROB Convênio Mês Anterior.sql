...
/*(SELECT ISNULL(SUM (I.SMM_QT),0)*/
			...
			WHERE  O.OSM_DTHR >= '2017-01-01 00:00' --DATEADD(D,1-DAY(GETDATE()),DATEADD(M,-1,CONVERT(CHAR(8),GETDATE(),112))) 
			AND O.OSM_DTHR < '2017-03-10 00:00' --DATEADD(D,1-DAY(getdate()),CONVERT(CHAR(8),getdate(),112))
 
			--AND I.SMM_SFAT <> 'C'
			--AND I.SMM_STR = 'EXL'
   			AND SMM_HON_SEQ = '2711' AND SMM_SFAT = 'F'
			--O.OSM_CNV = '990' AND SMM_SFAT = 'F'
			/*AND  O.OSM_CNV <> 'PCN' 
			AND  O.OSM_CNV <> 'PTL'
