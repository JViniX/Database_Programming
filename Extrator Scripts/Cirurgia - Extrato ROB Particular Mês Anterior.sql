...
select i.smm_osm_serie "OS Serie", i.smm_osm "OS Num", i.smm_sfat "Status Fat", i.smm_str "Cod. Setor", s.str_nome "Setor", 
	i.SMM_COD , p.SMK_NOME, i.smm_qt Qtd, pac.pac_reg, pac.pac_nome, i.smm_med "CRM", crm.psv_nome "Prestador", o.osm_cnv "Cod. Conv.", c.cnv_nome "Convenio", osm_pln_cod Plano, c.CNV_CAIXA_FATURA "Cobraca",
...
/*(SELECT ISNULL(SUM (I.SMM_QT),0)*/
			...
			WHERE  i.smm_osm = '75733' and
			O.OSM_DTHR >= '2016-06-01'--DATEADD(D,1-DAY(GETDATE()),DATEADD(M,-1,CONVERT(CHAR(8),GETDATE(),112))) 
			/*AND O.OSM_DTHR < getdate() --select DATEADD(D,1-DAY(getdate()),CONVERT(CHAR(8),getdate(),112))
